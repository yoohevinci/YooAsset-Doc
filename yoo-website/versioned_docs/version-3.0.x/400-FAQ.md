...
历史版本中，YooAsset 2.3.x 之前 WebGL 平台不支持资源文件加密。在构建选项里，可以将加密方法设置为空。

#### 问题：WebGL平台运行时报错：MissingMethodException : Default constructor not found for type YooAsset.WebRemoteFileSystem.
...
如果项目使用了扩展样例程序集，可以按需追加 `YooAsset.Extension`。
