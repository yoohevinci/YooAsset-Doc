...
#### 问题：ScriptableBuildTasks.cs脚本编译报错的问题

YooAsset依赖于ScriptBuildPipeline（SBP）的较新版本：1.21.x版本。

如果没有通过PackageManager来安装YooAsset，那么依赖的SBP库不会自动安装到本地。通过PackageManager来安装SBP库，如果没有匹配到1.21.x版本，可以先导入最新版本。然后找到工程目录下的Packages文件夹（Library文件夹同级目录），修改manifest.json文件。
...
YooAsset分为两部分，编辑器代码和运行时代码。因为工具界面是使用UIElements编写的，所以在Unity2019以前的版本是使用不了界面化工具。但是这并没有影响我们使用YooAsset运行时库。
...
#### 问题：打包的时候报错：System.Exception : "Found multiple BundleCollectorSetting files !"
...
private IEnumerator InitPackage()
{
    ...
    createParameters.BuiltinFileSystemParameters = null; //设置为NULL解决报错！
...
#### 问题：编辑器下运行时报错提示：Failed to load catalog file :yoo/DefaultPackage/BuiltinCatalog
...
这个问题的原因是EditorSimulateBuildInvoker.Build()方法每次调用都会重新生成一个用于模拟运行的资源清单，如果项目资源量巨大或者本地电脑CPU性能较弱，都会造成生成时间很久。
...
这是因为首次构建的时候，资源包构建界面里Copy Builtin File Option选项为Clear And Copy All。后面构建的时候，该选项没有变动，导致编辑器下的StreamingAssets目录下包含了最新的所有资源包。

YooAsset在检查更新的时候，会首先检查内置资源目录里是否存在，然后检查沙盒目录里是否存在，如果都不存在，就认为需要下载。

**注意：**编辑器下的沙盒目录存放在游戏工程的 `Library/yoo/` 目录下。
...
#### 问题：WebGL平台运行时报错：WebGL platform not support sync load method !

WebGL平台不支持用同步方法加载资源对象。可以切换为异步方法加载资源对象。

#### 问题：WebGL平台运行时报错：Failed to decompress data for the AssetBundle.

历史版本中，YooAsset 2.3.x 之前 WebGL 平台不支持资源文件加密。如果当前版本仍然出现该错误，请检查构建时的加密方式和运行时文件系统里配置的解密器是否匹配。

WebGL和小游戏平台当前支持RawBundle和ArchiveBundle加载，旧版本“不支持原生文件”的说明不再适用于当前版本。

#### 问题：WebGL平台运行时报错：MissingMethodException : Default constructor not found for type YooAsset.WebNetworkFileSystem.
...
如果项目使用了MiniGame或扩展样例程序集，可以按需追加 `YooAsset.MiniGame` 或 `YooAsset.Extension`。

#### 问题：小游戏平台运行时报错：Can not found file system class type : WechatFileSystem,YooAsset.MiniGame

通过添加link.xml配置来防止代码被裁剪。

```xml
<linker>
   <assembly fullname="YooAsset.MiniGame" preserve="all"/>
</linker>
```

#### 问题：真机上报错提示：The AssetBundle '__data' can't be loaded because another AssetBundle with the same files is already loaded.

一般出现这种报错是因为游戏里存在多个package资源包，解决办法是在Bundle Collector界面，把Unique Bundle Name勾选上即可。

如果只有一个package资源包，然后也收到了该报错信息。那么大概率是启动页面热更造成的，请确保在资源更新完之后，调用package.UnloadAllAssetsAsync()方法强制卸载所有AssetBundle。
...
如果在Bundle Collector窗口勾选了Enable Addressable选项，那么就是资源加载时填写的可寻址地址无效。
...
如果以上还没能解决问题，可以在编辑器下启用Editor Play Mode进行断点调试

```csharp
var package = YooAssets.GetPackage("PackageName");
package.IsLocationValid("location"); //断点位置，跟踪进去
```
...
#### 问题：真机上加载TMP字体文件失败：Failed to load asset : xxxTMPFont.asset

可以尝试将TMP字体和相关关联的资产打进一个AB里，例如TMP的材质球，TMP纹理，TMP关联的源字体。
...
private string GetMonoScriptsBundleName()
{
    ...
    var packRuleResult = DefaultBundlePackRule.CreateMonosPackRuleResult();
