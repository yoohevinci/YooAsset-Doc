...
- **Build Pipeline**
  ...
  (2) LegacyBuildPipeline: 旧版构建管线

  (3) ScriptableBuildPipeline: 可编程构建管线

  ​	Unity后续推出的可编程构建管线，和可编程渲染管线一样，让开发者有能力控制打包流程。

  ​	**注意：**从Unity2021.3版本开始，推荐使用该构建管线！

  (4) RawFileBuildPipeline: 原生文件构建管线
...
在Editor目录下实现一个继承IManifestEncryptor或IManifestDecryptor接口的类。
...
当构建完成后需要将差异化内容上传到CDN服务器，可以参考[示例代码](https://github.com/tuyoogame/YooAsset/tree/yoo3/Assets/YooAsset/Samples~/Extension%20Sample/Editor/PackageComparator)编写差异化比对工具。
...
下面是使用旧版构建管线来构建资源包的代码。

注意：可编程构建管线可以参考[源码](https://github.com/tuyoogame/YooAsset/tree/yoo3/Assets/YooAsset/Editor/BundleBuilder/VisualViewers/ScriptableBuildPipeline);
...
1. 构建APP的时候自动处理
   ...
   需要导入脚本到项目内：Samples/Extension Sample/Editor/PreprocessBuild（[示例代码](https://github.com/tuyoogame/YooAsset/tree/yoo3/Assets/YooAsset/Samples~/Extension%20Sample/Editor/PreprocessBuild)）

2. 手动人工生成

   需要导入脚本到项目内：Samples/Extension Sample/Editor/CreateBuiltinCatalog（[示例代码](https://github.com/tuyoogame/YooAsset/tree/yoo3/Assets/YooAsset/Samples~/Extension%20Sample/Editor/CreateBuiltinCatalog)）
...
- **Catalog文件**
  ...
  如果通过YOO的构建页面打包，并配置了BundledCopyOption导入内置文件，会通过TaskCreateCatalog任务节点自动生成Catalog文件。

  Catalog文件记录了资源包裹在StreamingAssets目录下的内置文件查询列表，它是默认的内置文件系统（BuiltinFileSystem）依赖的关键文件。
