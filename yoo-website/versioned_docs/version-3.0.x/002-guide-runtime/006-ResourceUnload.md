...
已移除 `UseWeakReferenceHandle` 初始化参数，资源句柄仍需要在不再使用时主动释放。
...
如果需要指定最大循环次数，使用 `UnloadUnusedAssetsOptions`：

```csharp
var options = new UnloadUnusedAssetsOptions(maxLoopCount);
var operation = package.UnloadUnusedAssetsAsync(options);
```

如果需要控制强制卸载行为，使用构造函数创建 `UnloadAllAssetsOptions`：

```csharp
var options = new UnloadAllAssetsOptions(shouldReleaseHandles: true, shouldLockLoading: true);
var operation = package.UnloadAllAssetsAsync(options);
```
...
IEnumerator Start()
{
    ...
    var operation = sceneHandle.UnloadSceneAsync();
    yield return operation;
}
```
