...
在安卓平台加载包体内的加密文件或原生文件时，内置文件系统会先把文件拷贝到沙盒内，再做后续加载或缓存处理。

当前版本不再提供本地文件拷贝服务接口。如果项目需要完全接管包体内文件的读取、拷贝或缓存逻辑，建议通过自定义文件系统来实现平台相关处理。

常见处理方式如下：

1. 继续使用默认内置文件系统，由YooAsset完成包体内文件到沙盒目录的拷贝。
2. 如果需要导入外部下载或解压后的资源文件，使用资源导入器导入到沙盒缓存。
3. 如果平台文件访问方式有特殊要求，扩展文件系统并在初始化参数中接入自定义文件系统。
...
详情参考 [UniTask 配置教程](https://github.com/tuyoogame/YooAsset/blob/yoo3/Assets/YooAsset/Samples~/UniTask%20Sample/README.md)
